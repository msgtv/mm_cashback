javascript: (function() {RewardCashback
  ...
  const RewardDicount = 'DSCNT';
  const RewardCashback = 'CSHBCK';

  const whatFind = 'best-cash-id-what-find';
  const percentId = 'best-cash-id-min-percent';
  const delayId = 'best-cash-id-delay';
  const pageId = 'best-cash-id-max-page';
  const submitBtn = 'best-cash-id-submit';
  ...
  function getInput() {
    ...
    overlay.id = "best-cash-id-overlay";
    ...
    let formHtml = `<label for="${whatFind}">Что ищем?</label><select name="select" id="${whatFind}"><option value="${RewardCashback}" selected>Кэшбэк</option><option value="${RewardDicount}">Скидку</option></select><label for="${percentId}"">Минимальный процент:</label><input type="number" id="${percentId}" name="min-percent" value="" required"><label for="${delayId}"">Задержка (оптимальное - 3 сек.):</label><input type="number" id="${delayId}" name="delay" value="3" required"><label for="${pageId}">Страниц:</label><input type="number" id="${pageId}" name="max-page""><input id="${submitBtn}" type="submit" value="СТАРТ">`;
